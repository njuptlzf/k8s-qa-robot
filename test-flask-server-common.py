...
service_name="lzf图书馆管理系统"
service_description=f"在某服务器的 k3s 集群 namespace=lzf 有一套'lzf图书馆管理系统'服务, statefuleset包含 mysql, redis; deployment包含 library-api, library-web。"
robot_name="贾维斯"
...
messages.append({"role": "system", "content": """你是中文专家，请你充分学习中文动词，形容词，语气助词等语义"""} )
...
messages.append({"role": "system", "content": """当我提问'重启'、请重启、请你、请完成、请处理、怎么做、请操作、请安装、请升级、请重启、请配置、请检查、请解决等相近的语义词时，你回答时要包含commands字段"""} )

def run_command(msg: list, commands):
    try:
        error_message=""
        result = subprocess.run(commands, shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            success_message = f"执行命令成功: {commands}"
            msg.append({"role": "assistant", "content": success_message})
        ...
        msg.append({"role": "assistant", "content": error_message})
        print(error_message)
...
def ask():
    ...
    if commands and len(commands) > 0:
        if len(description) == 0:
            description=f"正在执行命令: {commands}"
            print(description)
            messages.append({"role": "assistant", "content": f"{msg}"})
        messages, error_message=run_command(messages,commands)
        if error_message and error_message.strip != "":
            return jsonify(error_message)
    else:
        print(description)
    # 将返回值改为 jsonify(messages[-1])
